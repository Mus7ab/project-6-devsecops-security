...
nano docs/findings.md
- **Helm test-connection pod (`wget`/busybox container) — UPDATE (Phase 6): Now remediated.** Originally deprioritized here as a transient `helm test` hook with a small attack window. However, the CI/CD pipeline built in Phase 6 (`docs/ci-cd-pipeline.md`) independently re-detected this exact gap via automated HIGH/CRITICAL gating on its first live run, correctly refusing to pass despite the earlier deprioritization decision. Rather than suppress the finding, the pod's `securityContext`/`podSecurityContext` was added directly to `templates/tests/test-connection.yaml`, matching the main Deployment's remediation pattern. Verified via local Trivy rescan (0 HIGH/CRITICAL remaining) and confirmed live on a second GitHub Actions run, which passed all 5 jobs. This demonstrates the value of automated gating catching gaps that manual triage had deliberately (and reasonably, at the time) set aside.
